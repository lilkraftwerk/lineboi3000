...
module.exports = function(cncserver) {
            ...
            cncserver.botConf
                ...
                .load(function() {
                    ...
                    if (overrides) {
                        if (overrides[botType]) {
                            for (const key in overrides[botType]) {
                                cncserver.botConf.set(
                                    key,
                                    overrides[botType][key]
                                );
                            }
                        }
                    }
                    ...
                    cncserver.bot = {
                        workArea: {
                            left: Number(
                                cncserver.botConf.get('workArea:left')
                            ),
                            top: Number(cncserver.botConf.get('workArea:top')),
                            right: Number(
                                cncserver.botConf.get('maxArea:width')
                            ),
                            bottom: Number(
                                cncserver.botConf.get('maxArea:height')
                            )
                        },
                        maxArea: {
                            width: Number(
                    ...
                    const swapMotors = cncserver.botConf.get(
                        'controller:swapMotors'
                    );
    ...
    cncserver.settings.getSupportedBots = function() {
        ...
        const list = fs.readdirSync(
            path.resolve(__dirname, '..', 'machine_types')
        );
        const out = {};
        for (const i in list) {
